...
public class EnemyController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        this.GetComponent<Animator>().Play("EnemyAnimation1");
    ...
    void Update()
    {
        if (firstTime)
        {
            transform.position = Vector2.MoveTowards(transform.position, new Vector3(0, 26.9f), Time.deltaTime * 5);
            if (transform.position == new Vector3(0, 26.9f)) firstTime = false;
    ...
    IEnumerator turnAnimOff()
    {
        yield return new WaitForSeconds(9f); this.GetComponent<Animator>().enabled = false; transform.position = new Vector3(0, 26.9f, 0); this.originalY = this.transform.position.y;
    }
}
